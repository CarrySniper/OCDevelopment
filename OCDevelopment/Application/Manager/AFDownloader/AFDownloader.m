...
#import "AFDownloader+PlistFile.h"
#import "AFDownloader+DataListener.h"
		 ...
		 completion:(CLDownloadCompletionHandler)completion
{
	...
	AFDownloaderObject *object = [[AFDownloaderObject alloc]initWithUrlString:encodedString beginLocation:length directoryPath:directory];
	if (object == nil) {
		return;
	}
	...
	object.stateHandler = state;
	object.progressHandler = progress;
	object.completionHandler = completion;
...
- (void)toDowloadNextObject {
    ...
    AFDownloaderObject *object = self.waitingArray.firstObject;
    ...
    if ([self canBeginDownload]) {
        ...
        if (object.stateHandler) {
            object.stateHandler(CLDownloadStateRunning);
        }
    }else{
        for (AFDownloaderObject *downloadObjec in self.waitingArray) {
            if (downloadObjec.stateHandler) {
                downloadObjec.stateHandler(CLDownloadStateWaiting);
            }
        }
    }
}
...
- (void)suspendDownload:(NSString *)urlString {
    ...
    AFDownloaderObject *object = self.downloadsSet[CLFileName(URL)];
    if (!object) {
        return;
    }
    ...
    if (object.stateHandler) {
        object.stateHandler(CLDownloadStateSuspended);
    }
...
- (void)suspendAllDownloads {
    ...
    if (self.waitingArray.count > 0) {
        for (AFDownloaderObject *object in self.waitingArray) {
            if (object.stateHandler) {
                object.stateHandler(CLDownloadStateSuspended);
            }
        }
...
- (void)resumeAllDownloads {
    ...
    for (AFDownloaderObject *object in downloads) {
        [self.waitingArray addObject:object];
        [self toDowloadNextObject];
    }
}
...
- (void)deleteDownload:(NSString *)urlString {
    ...
    if (object.stateHandler) {
        object.stateHandler(CLDownloadStateCanceling);
    }
...
- (void)deleteAllDownloads {
    ...
    for (AFDownloaderObject *object in objects) {
        [object closeOutputStream];
        [object.dataTask cancel];
        if (object.stateHandler) {
            object.stateHandler(CLDownloadStateCanceling);
...
- (NSString *)cachesDirectory {
	NSString *cachesDirectory = objc_getAssociatedObject(self, _cmd);
	if (!cachesDirectory) {
		NSString *caches = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject];
		// 到文件夹路径
		cachesDirectory = [caches stringByAppendingPathComponent:NSStringFromClass([self class])];
		...
		objc_setAssociatedObject(self, _cmd, cachesDirectory, OBJC_ASSOCIATION_RETAIN);
	}
...
- (NSMutableDictionary<NSString *,AFDownloaderObject *> *)downloadsSet {
	if (!_downloadsSet) {
		// 读取Plist存储信息，转化为下载对象
